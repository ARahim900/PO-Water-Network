...
/** The 3D explorer, unchanged in behaviour: the decision-support shell wraps it and owns the page header and footer. */
export default function Explorer() {
 ...
 return <div>
  <section className="flex items-center justify-between gap-3 px-4 py-3 md:px-8 md:py-5"><div><h2 className="text-xl md:text-2xl">3D model of both installations</h2><p className="mt-1 hidden md:block">Inspect both installation options. Select a component to fly closer.</p></div><div className="flex flex-col items-end gap-1"><div className="flex gap-2"><button aria-label="Open full-screen 3D" onClick={()=>{setViewerControls(false);setExpanded(true);}} className="control md:hidden"><Maximize size={20}/></button><button aria-label="Copy link to this view" onClick={copyLink} className="control"><Link2 size={20}/><span className="hidden sm:inline">Copy link</span></button><button aria-label="Download static 3D view" onClick={()=>setExportSerial(v=>v+1)} className="control"><Download size={20}/><span className="hidden sm:inline">3D export</span></button></div><p aria-live="polite" className="min-h-4 text-right text-[13px] text-purple">{linkNote}</p></div></section>
